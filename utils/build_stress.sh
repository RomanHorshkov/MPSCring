#!/usr/bin/env bash
# =============================================================================
# build_stress.sh — build the multi-producer stress test against the release
...
gcc "${CPPFLAGS_RELEASE[@]}" -DMPSC_REQUIRE_ALWAYS_LOCK_FREE -D_GNU_SOURCE -Iapp \
    "${CFLAGS_RELEASE[@]}" -c tests/stress/stress_mt.c -o "${BUILD_DIR}/stress_mt.o"
gcc "${LDFLAGS_RELEASE[@]}" "${BUILD_DIR}/stress_mt.o" build/release/libmpscring.a \
