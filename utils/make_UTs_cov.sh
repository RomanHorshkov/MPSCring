...
gcc -std=c11 -O0 -g "${COVERAGE_FLAGS[@]}" -DMPSC_RING_TESTING -Iapp -Itests/UTs \
    -c app/mpscring.c -o "${BUILD_DIR}/mpscring.o"

# Build unit test objects.
UT_CFLAGS=(-std=c11 -O0 -g "${COVERAGE_FLAGS[@]}" -D_GNU_SOURCE -DMPSC_RING_TESTING -Iapp -Itests/UTs)
