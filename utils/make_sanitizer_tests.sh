...
build_binary() {
    ...
    local objects=("${BUILD_DIR}/mpscring_${suite_name}.o")
    local source output

    gcc "${SANITIZE_CPPFLAGS[@]}" "${SANITIZE_CFLAGS[@]}" -c app/mpscring.c -o "${objects[0]}"
...
gcc "${CPPFLAGS_SANITIZE[@]}" -DMPSC_REQUIRE_ALWAYS_LOCK_FREE -D_GNU_SOURCE \
    -DN_PER_PRODUCER=50000 -Iapp "${SANITIZE_CFLAGS[@]}" \
