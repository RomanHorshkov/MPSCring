...
install -m 0755 "build/release/libmpscring.so.$VER" "$STAGE/usr/local/lib/libmpscring.so.$VER"
"$STRIP" --strip-unneeded "$STAGE/usr/local/lib/libmpscring.so.$VER"
ln -sf "libmpscring.so.$VER" "$STAGE/usr/local/lib/libmpscring.so.$MAJOR"
ln -sf "libmpscring.so.$VER" "$STAGE/usr/local/lib/libmpscring.so"

install -m 0644 build/release/libmpscring.a "$STAGE/usr/local/lib/libmpscring.a"
...
"${ROOT_DIR}/utils/check_hardening.sh" "$STAGE/usr/local/lib/libmpscring.so.$VER"
