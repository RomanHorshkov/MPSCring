#!/usr/bin/env bash
# =============================================================================
# build_libs.sh — build libmpscring across the shared GCC profile catalog
#
...
#
#   build/<profile>/libmpscring.so.<VERSION>   (+ .so / .so.<MAJOR> symlinks)
#   build/<profile>/libmpscring.a
#
...
# --- project description ------------------------------------------------------
LIB_BASENAME="mpscring"
LIB_SOURCES=(app/mpscring.c)
LIB_CPPFLAGS=(-Iapp -DMPSC_REQUIRE_ALWAYS_LOCK_FREE)
# mpscring is C11-atomics + libc only; with -Wl,-z,defs any additional needed
...
build_profile() {
    ...
    gcc "${LDFLAGS_SHARED[@]}" "${extra_ldflags[@]}" \
        ...
        -Wl,--version-script,"${ROOT_DIR}/utils/mpscring.map" \
        -o "${shared_path}" \
        "${pic_objects[@]}" \
